...
func _subdivide() -> void:
	...
	if !_objects.empty():
		...
		for body in existing_objects:
			_remove_qt_metadata(body)
			if !(body is VisualInstance) and body.has_meta("_bounds"):
...
#		print(object)
		...
		if !(object is VisualInstance) and object.has_meta("_bounds"):
			transformed_aabb = object.get_meta("_bounds")
		else:
			transformed_aabb = object.get_transformed_aabb()
#		print(bound.encloses(transformed_aabb))
